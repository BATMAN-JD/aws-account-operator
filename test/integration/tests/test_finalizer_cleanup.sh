...
finalizerClaimName="${FINALIZER_CLAIM_NAME:-test-finalizer-claim}"
finalizerNamespace="${FINALIZER_NAMESPACE_NAME:-test-finalizer}"
...
function setup {
    echo "========================================================================="
    echo "SETUP: Creating namespace and AccountClaim"
    echo "========================================================================="
...
function test {
    ...
    timeoutSeconds=$(echo "${timeout}" | sed 's/m/*60/;s/s//;s/h/*3600/' | bc)
    local elapsed=0
...
function cleanup {
    ...
    if oc get accountclaim "${finalizerClaimName}" -n "${finalizerNamespace}" &>/dev/null; then
        ...
        deleteAccountClaimCR "${finalizerClaimName}" "${finalizerNamespace}" "${RESOURCE_DELETE_TIMEOUT}" true 2>/dev/null || {
    ...
    deleteNamespace "${finalizerNamespace}" "${RESOURCE_DELETE_TIMEOUT}" true 2>/dev/null || {
